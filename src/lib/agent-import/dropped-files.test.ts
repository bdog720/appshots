...
describe("filesFromEntries", () => {
  it("walks nested directories across reader batches", async () => {
    const files = await filesFromEntries([
      dirEntry("set", [
        [fileEntry("breezel.json"), dirEntry("shots", [[fileEntry("01.png")], [fileEntry("02.png")]])],
        [fileEntry("badge.png")],
      ]),
    ]);
    expect(files.map((f) => f.name)).toEqual(["breezel.json", "01.png", "02.png", "badge.png"]);
  });
});

describe("filesFromDataTransfer", () => {
  it("falls back to plain files when entries are unavailable", async () => {
    const file = new File(["x"], "breezel.json");
  ...
  it("uses webkitGetAsEntry when available", async () => {
    const dataTransfer = {
      items: [{ webkitGetAsEntry: () => fileEntry("breezel.json") }],
      files: [],
    } as unknown as DataTransfer;
    const files = await filesFromDataTransfer(dataTransfer);
    expect(files.map((f) => f.name)).toEqual(["breezel.json"]);
  });
});
